...
describe('AI Mode discloses when a "successful" send was actually simulated', () => {
  ...
  beforeAll(async () => {
    ...
    customer = await Customer.create({ firstname: '__test_ai_disclosure__', lastname: 'Customer', phone: '15559990299', isDemo: true, workspaceId });
    promo = await Promotion.create({ name: '__test_ai_disclosure_promo__', scope: 'products', customerType: 'cash', isDemo: true, workspaceId });
  }, 30000);
